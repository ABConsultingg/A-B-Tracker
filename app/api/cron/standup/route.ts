...
const TEMPLATE = process.env.TWILIO_TEMPLATE_STANDUP;
...
export async function GET(req: NextRequest) {
  ...
  if (!TEMPLATE) console.error('[cron/standup] TWILIO_TEMPLATE_STANDUP not set — no WhatsApp sent');
  for (const r of results) {
    if (r.status === 'rejected') console.error('[cron/standup] WhatsApp failed:', r.reason);
  }
